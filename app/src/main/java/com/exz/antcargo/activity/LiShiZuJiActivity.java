...
public class LiShiZuJiActivity extends BaseActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener {
    ...
    private LookListAdapter<MyLookListBean> lookListAdapter;//查看记录的adapter

    private ClickListAdapter<ClickBean> clickListAdapter;//我被点击的记录

    private Context c = LiShiZuJiActivity.this;
    ...
    public void initView() {
        ...
        lookListAdapter = new LookListAdapter<MyLookListBean>(LiShiZuJiActivity.this);
        lv_01.setAdapter(lookListAdapter);
        clickListAdapter = new ClickListAdapter<ClickBean>(LiShiZuJiActivity.this);
        ...
        xUtilsApi.sendUrl(c, "post", params, true, new XUtilsApi.URLSuccessListenter() {
            ...
            public void OnSuccess(NewEntity content, JSONObject result) {
                if (content.getResult().equals(ConstantValue.RESULT)) {
                    ...
                    lookListAdapter.addendData(JSON.parseArray(optJSONObject, MyLookListBean.class), true);
                    lv_01.setAdapter(lookListAdapter);
    ...
    private void myClickList(String url) {
        ...
        xUtilsApi.sendUrl(c, "post", params, true, new XUtilsApi.URLSuccessListenter() {
            ...
            public void OnSuccess(NewEntity content, JSONObject result) {
                if (content.getResult().equals(ConstantValue.RESULT)) {
                    String optJSONObject = result.optString("info");
                    clickListAdapter.addendData(JSON.parseArray(optJSONObject, ClickBean.class), true, "车主");
                    lv_02.setAdapter(clickListAdapter);
