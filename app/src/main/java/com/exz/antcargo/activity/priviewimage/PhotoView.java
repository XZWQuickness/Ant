package com.exz.antcargo.activity.priviewimage;
...
public class PhotoView extends ImageView
{
	 ...
	 */
	public void finishActivity(Activity activity, ClickListener clickListener)
	{
	...
	private class Transform implements Runnable
	{
		...
		void withScale(float form, float to)
		{
			mScaleScroller.startScroll((int) (form * 10000), 0,
					(int) ((to - form) * 10000), 0, ANIMA_DURING);
		}
		...
		void withRotate(int fromDegrees, int toDegrees)
		{
			mRotateScroller.startScroll(fromDegrees, 0,
					toDegrees - fromDegrees, 0, ANIMA_DURING);
		}
	...
	public Info getInfo()
	{
		...
		return new Info(rect, local, mImgRect, mWidgetRect, mScale, mDegrees,
				mScaleType);
	}
	...
	public void animaFrom(Info info)
	{
		...
		if (isInit)
		{
			...
			mAnimaMatrix.reset();
			mAnimaMatrix.postTranslate(-mBaseRect.left, -mBaseRect.top);
			mAnimaMatrix.postTranslate(ocx - mBaseRect.width() / 2, ocy
					- mBaseRect.height() / 2);
			...
			mTranslate.withTranslate(0, 0, (int) (mScreenCenter.x - ocx),
					(int) (mScreenCenter.y - ocy));
